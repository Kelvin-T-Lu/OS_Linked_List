...
static void print_process(Process *queue);
...
/* Stage 1) Perform 1 action from the trace file  */
    ...
    /* If kill... */
      ...
      if(ret_code & 0x10) { // STOP FLAG was set
        stop_flag = 1;
        ret_code &= ~(0x10);
        on_cpu = NULL;
      }
...
/* Stage 2) Unload the Running Process from CPU and Return to Scheduler */
    ...
    else if(stop_flag == 0) { /* Nothing was on CPU */
      printf("+------------\n");
 ...
 */
static void print_process(Process *process) {
  char flags[255] = {0};
  if(process == NULL) {
    printf("|\tNone\n");
  } else {
      load_flags(flags, process->flags);
      printf("|\tPID: %3d Time (Rem: %2d, Last Run: %2d), Wait Rem: %2d, Flags:%s, Command: %s\n", 
             process->pid, process->time_remaining, process->time_last_run,
             process->wait_remaining, flags, process->command);
  }
  return;
}
...
static int parse_kill(Schedule *schedule, char *trace_buffer, Process *on_cpu) {
  ...
  if(do_stop) {
    if(on_cpu == NULL) {
      ...
      if(pid != on_cpu->pid) {
        printf("| Error: STOP PID (%2d) does not match Running PID (%2d)\n", pid, on_cpu->pid);
        ret_code = -1;
      }
      else {
        ...
        print_process(on_cpu);
        ret_code |= 0x10; // SETTING STOP FLAG
      }
    }
  }
  else if(do_cont) {
